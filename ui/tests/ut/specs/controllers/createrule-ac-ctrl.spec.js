...
function(angular, mocks, CreateRuleACCtrl) {
    describe('Test /js/controllers/createrule-ac-ctrl.js',
    function() {
        beforeEach(function() {
        ...
        beforeEach(function() {
            $scope = $rootScope.$new();
            controller = $controller('CreateRuleACCtrl', {
                $scope: $scope,
                $route: $route,
                toaster: toaster
            });
        ...
        describe("if the controller of CreateRuleACCtrl exists",
        function() {
        ...
        describe("check if function are work",
        function() {

            it('$scope.selectNodeLabelTarget  works well',
            function() {
                expect($scope.selectNodeLabelTarget).toBeDefined();
            });

            it('$scope.toggleSelection works well',
            function() {
                expect($scope.toggleSelection).toBeDefined();
            });

            it('$scope.toggleAll works well',
            function() {
                expect($scope.toggleAll).toBeDefined();
            });

            it('$scope.form is a object',
            function() {
        ...
        describe("httpGet $config.uri.dbtree test",
        function() {
            beforeEach(function() {
                $httpBackend.when('GET', $config.uri.dbtree).respond({
                    "age": 16,
                    "name": "li"
                });
                $httpBackend.when('GET', $config.uri.schemadefinition).respond({
                    "age": 16,
                    "name": "li"
                });
            });
            ...
            it('test watch and http request',
            function() {
                $scope.currentNode = {
                    "name": "ha"
                };

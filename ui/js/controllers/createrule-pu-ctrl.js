...
function(controllers) {
    'use strict';
    controllers.controller('CreateRulePUCtrl', ['$scope', '$http', '$config', '$location', 'toaster', '$timeout', '$route', '$filter',
        ...
        $http.get(dbtreeUrl).then(function successCallback(data) {
            ...
            function(p) {
                if (p.platform == 'Apollo') { //::TODO:Only Apollo is supported cussrently
                    $scope.allSystem = {};
                    angular.forEach(p.systems,
                    function(sys) { //retrieve all systems
